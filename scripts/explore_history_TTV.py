...
if __name__ == "__main__":
    ...
    for key in planets_keys:
        ...
        #planet_rotation_period = 2*np.pi / planet_norm_spin  / 24.0                                         # Uncomment for rotation period plot at (5,3,6)
        planet_rotation_period = planet_orbital_period / (2*np.pi / planet_norm_spin) / (86400.0 * 24.0)      # Uncomment for omega / n plot at (5,3,6)
        ...
        non_zero = e != 0
        alpha = np.ones(len(e))
        alpha[non_zero] = (1.+15./2.*e[non_zero]**2+45./8.*e[non_zero]**4+5./16.*e[non_zero]**6)*1./(1.+3.*e[non_zero]**2+3./8.*e[non_zero]**4)*1./(1.-e[non_zero]**2)**1.5 # Eq. 42 of Hut 1981
        ...
        #pseudo_synchronization_period = alpha[non_zero]
...
#    plt.savefig(output_figure_filename)       # uncomment for plotting (not very useful for short timespan regarding TTV analysis)
#    plt.show() # uncomment for plotting (not very useful for short timespan regarding TTV analysis)
#    print("> Output figure file written to: {}".format(output_figure_filename)) # uncomment for plotting (not very useful for short timespan regarding TTV analysis)
    ...
    for key in planets_keys:
        ...
        data['position_x'] = planet_data['position_x']
        data['position_y'] = planet_data['position_y']
